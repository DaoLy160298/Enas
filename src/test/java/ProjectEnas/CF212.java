...
public class CF212 extends BaseTest {
	...
	public void TC01a_Check_UI_Menu() {
		...
		demoDrapDropPage.drapAndDropPosition("規約に同意する");
		log.info("Check the text title of the functional component in the menu");
		verifyTrue(demoDrapDropPage.isDisplayTitleTextLabelMenu("規約に同意する"));
		...
		verifyTrue(demoDrapDropPage.checkInputValueMenu("タイトル","規約に同意する"));
		verifyTrue(demoDrapDropPage.checkInputValueMenu("サンプルテキスト","規約の内容を入力してください。"));
		...
		Assert.assertEquals(demoDrapDropPage.getColorTexLabel("規約に同意する"),"#333D5A");
	...
	public void TC01b_Check_UI_Form() {
		...
		log.info("Check the default When entering the screen");
		verifyTrue(demoDrapDropPage.checkChangeValueTitleForm("規約に同意する"));
		verifyTrue(demoDrapDropPage.checkValueSampleTextForm("規約の内容を入力してください。"));
