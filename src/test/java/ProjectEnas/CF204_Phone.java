...
public class CF204_Phone extends BaseTest {
	...
	public void TC01a_Check_UI_Menu() {
		...
		demoDrapDropPage.drapAndDropPosition("電話番号");
		log.info("Check the text title of the functional component in the menu");
		verifyTrue(demoDrapDropPage.isDisplayTitleTextLabelMenu("電話番号"));
		...
		verifyTrue(demoDrapDropPage.checkInputValueMenu("タイトル","電話番号"));
		verifyTrue(demoDrapDropPage.checkInputValueMenu("サンプルテキスト","0987654321"));
		...
		Assert.assertEquals(demoDrapDropPage.getColorTexLabel("電話番号"),"#333D5A");
		...
		Assert.assertEquals(demoDrapDropPage.getFontSizeTextLabel("電話番号"),"18px");
	...
	public void TC01b_Check_UI_Form() {
		...
		log.info("Check the default When entering the screen");
		verifyTrue(demoDrapDropPage.checkChangeValueTitleForm("電話番号"));
		verifyTrue(demoDrapDropPage.checkValueSampleTextForm("0987654321"));
