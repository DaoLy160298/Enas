...
public class CF201_Email extends BaseTest {
	...
	public void TC01a_Check_UI_Menu() {
		...
		demoDrapDropPage.drapAndDropPosition("メールアドレス");
		log.info("Check the text title of the functional component in the menu");
		verifyTrue(demoDrapDropPage.isDisplayTitleTextLabelMenu("メールアドレス"));
		...
		verifyTrue(demoDrapDropPage.checkInputValueMenu("タイトル", "メールアドレス"));
		verifyTrue(demoDrapDropPage.checkInputValueMenu("サンプルテキスト", "mail@example.com"));
		...
		Assert.assertEquals(demoDrapDropPage.getColorTexLabel("メールアドレス"), "#333D5A");
		...
		Assert.assertEquals(demoDrapDropPage.getFontSizeTextLabel("メールアドレス"), "18px");
	...
	public void TC01b_Check_UI_Form() {
		...
		log.info("Check the default When entering the screen");
		verifyTrue(demoDrapDropPage.checkChangeValueTitleForm("メールアドレス"));
		verifyTrue(demoDrapDropPage.checkValueSampleTextForm("mail@example.com"));
		...
		Assert.assertEquals(demoDrapDropPage.getColorSampleTextForm(), "#C0C3CC");
...
//		Assert.assertEquals(demoDrapDropPage.getColorAdditionalExForm(),"#C0C3CC");
		...
		Assert.assertEquals(demoDrapDropPage.getFontSizeSampleTextForm(), "14px");
	...
	@Test
	public void TC_07_Duplicate_Component() {
		demoDrapDropPage.clickIconDuplicate("copy", times);
		verifyTrue(demoDrapDropPage
				.checkCountTitleComponent(userForm.getTitleComponents().get(8).getTitleComponentText(), (times + 1)));
		verifyTrue(
				demoDrapDropPage.checkCountTextSample(userForm.getSampleTexts().get(8).getSampleText(), (times + 1)));
		verifyTrue(demoDrapDropPage.checkCountGeneralEx(userForm.getGeneralExplantions().get(8).getGeneralExplantion(),
				(times + 1)));
		verifyTrue(demoDrapDropPage.checkCountAdditionalEx(
				userForm.getAdditionalExplantions().get(8).getAdditionalExplantion(), (times + 1)));
	}

	@Test
	public void TC_08_Delete_Component() {
		demoDrapDropPage.clickIconDelete("delete", times);
		verifyTrue(demoDrapDropPage
				.checkCountTitleComponent(userForm.getTitleComponents().get(8).getTitleComponentText(), 1));
		verifyTrue(
				demoDrapDropPage.checkCountTextSample(userForm.getSampleTexts().get(8).getSampleText(), 1));
		verifyTrue(demoDrapDropPage.checkCountGeneralEx(userForm.getGeneralExplantions().get(8).getGeneralExplantion(),
				1));
		verifyTrue(demoDrapDropPage.checkCountAdditionalEx(
				userForm.getAdditionalExplantions().get(8).getAdditionalExplantion(), 1));
	}

	 @Test
	public void TC_09_Move_Up_Component() {
		verifyTrue(demoDrapDropPage.checkPositionAfterMove(userForm.getTitleComponents().get(8).getTitleComponentText(),"move up",userForm.getTitleComponents().get(8).getTitleComponentText()));
		
	}
	 @Test
	public void TC_10_Move_Down_Component() {
		verifyTrue(demoDrapDropPage.checkPositionAfterMove(userForm.getTitleComponents().get(8).getTitleComponentText(),"move down",userForm.getTitleComponents().get(8).getTitleComponentText()));
