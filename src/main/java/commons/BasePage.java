...
public class BasePage {
	...
	public void switchToWindowByID(WebDriver driver, String windowID) {
		...
		// Lặp qua từng window handle để chuyển đổi giữa các tab
	...
	public void switchToWindowByUrl(WebDriver driver, String url) {
		...
		for (String id : allWindowIDs) {
			driver.switchTo().window(id);
			String actualPageTitleString = driver.getCurrentUrl();
			System.out.println(actualPageTitleString);
			if (actualPageTitleString.contains(url)) {
				break;
			}
		}
	}
	public void openNewTab(WebDriver driver,String url) {
		 // Sử dụng JavaScript Executor để mở tab mới
        JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
        jsExecutor.executeScript("window.open()");

        // Lấy danh sách các tab
        // Chuyển đổi qua tab mới bằng cách chọn tab cuối cùng trong danh sách
	...
	public void clearValueInElementPressKey(WebDriver driver, String locatorType, String... dynamicValues) {
...
//		select.selectByValue(textItem);
	...
	public void selectItemInCustomDropdown(WebDriver driver, String parentXpath, String dynamicValueParent,
			String childXpath, String dynamicValueChildren, String expectedText) {
		parentXpath = getDynamicXpath(parentXpath, dynamicValueParent);
		WebElement parentElement = getWebElement(driver, parentXpath);

		// Scroll đến phần tử cha trước khi click
		JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
		jsExecutor.executeScript("arguments[0].scrollIntoView(true);", parentElement);
		sleepInSecond(1);

		parentElement.click();

		childXpath = getDynamicXpath(childXpath, dynamicValueChildren);
		WebDriverWait explicitWait = new WebDriverWait(driver, Duration.ofSeconds(longTimeout));

		// Scroll đến phần tử con trước khi xử lý
		WebElement childElement = explicitWait
				.until(ExpectedConditions.presenceOfElementLocated(getByLocator(childXpath)));
...
//	public void selectItemInCustomDropdown(WebDriver driver, String parentXpathTemplate, String childXpathTemplate,
//			String dynamicValueForParent, String dynamicValueForChild, String expectedText) {
//		String parentXpath = String.format(parentXpathTemplate, dynamicValueForParent);
//		String childXpath = String.format(childXpathTemplate, dynamicValueForChild);
//
//		getWebElement(driver, parentXpath).click();
//		sleepInSecond(1);
//		WebDriverWait explicitWait = new WebDriverWait(driver, Duration.ofSeconds(longTimeout));
...
//	}
	...
	public String getTextInputRequest(WebDriver driver) {
		JavascriptExecutor js = (JavascriptExecutor) driver;
		String validationMessage = (String) js
				.executeScript("return document.querySelector('input[required]').validationMessage");
	...
	public String getFontSize(WebDriver driver, String locatorType) {
		return getWebElement(driver, locatorType).getCssValue("font-size");
	}

	public String getFontSize(WebDriver driver, String locatorType, String... dynamicValues) {
		return getWebElement(driver, getDynamicXpath(locatorType, dynamicValues)).getCssValue("font-size");
	}

	public String getColor(WebDriver driver, String locatorType) {
		return getWebElement(driver, locatorType).getCssValue("color");
	}

	public String getColor(WebDriver driver, String locatorType, String... dynamicValues) {
		return getWebElement(driver, getDynamicXpath(locatorType, dynamicValues)).getCssValue("color");
	}
...
//		waitForElementClickable(driver, CommonUI.DYNAMIC_PAGE_MENU, pageName);
//		clickToElement(driver, CommonUI.DYNAMIC_PAGE_MENU, pageName);
//		switch (pageName) {
//		case "Dashboard":
...
//		case "Products":
//			return PageGeneratorManager.getListProduct(driver);
////		case "New Account":
...
//	}
	...
	public String getTextRequestBlank(WebDriver driver, String locatorType) {
		return getWebElement(driver, locatorType).getAttribute("validationMessage");
	}
...
//	}
//	
	public CommonPO clickLogoButton(WebDriver driver) {
	...
	public void drapAndDropComponent(WebDriver driver, String locatorType,String dropPosition, String... dynamicValues) {
		locatorType = getDynamicXpath(locatorType, dynamicValues);
